...
ui <- page_fillable(
  ...
  navset_card_tab(
    ...
    nav_panel(
      ...
      card_body(
        markdown("This 𝒮𝒽𝒾𝓃𝓎 app is the first in the **Glossonaut** collection of online interfaces for exploring linguistic databases (see also [Glossonaut PHOIBLE](https://stephen-nichols.shinyapps.io/glossonaut-phoible/)). It provides an alternative way of examining the data underpinning the [World Atlas of Language Structures (WALS)](https://wals.info/). It uses the R package [`ritwals`](https://hkauhanen.github.io/ritwals/) by [Henri Kauhanen](https://hkauhanen.fi/) to import the WALS data.
        
        The **\"Languages Map\"** tab produces a map of languages filtered according to the input in the sidebar. If you select mutually exclusive combinations, no languages will be found! For example, selecting the family \"Panoan\" will display no languages if the data set is also filtered to \"WALS 100\" (as is the default). Filtering instead to \"WALS 200\" will show only Shipibo-Konibo, the only Panoan language in the WALS 200 list, and choosing \"All languages\" will output all eleven Panoan languages (unless other features are changed). This same logic applies to language, geographic and feature filters alike. This same filtered data can be viewed in the **\"Languages Table\"** and **\"Features Table\"** tabs. These two tabs are capable of showing the same data just in different formats: \"Languages Table\" uses a wide format whereas \"Features Table\" uses a long format. The \"Languages Table\" tab hides features and feature values by default but these can be shown as additional columns. The \"Features Table\" tab hides absent feature values by default but these can be shown as additional rows.
        
        On the map, language markers are currently colour-coded according to which family each language belongs, though, due to the limited number of colours, available in [`leaflet`](https://cran.r-project.org/web/packages/leaflet/), each colour is assigned to multiple families. Implementing colour-coding for other options is something on the table for future versions of Glossonaut WALS.
        
        For more information on the data viewable here, please consult [WALS](https://wals.info/) itself.
